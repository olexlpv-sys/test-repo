...
- FR-V7 **[A]** Only the document owner can create a new draft, discard a draft, rename and delete the document; owner or admin can move it. The title is document-level (A-1). Discarding a draft is **rejected (`409 only-version`) when no signed version exists** — the owner deletes the document instead (A-2).
