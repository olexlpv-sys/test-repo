...
- FR-UI2 Clicking a document row opens the document form: the document is shown as one Word-like page of **sections (one per node)** with the tree on the left for navigation and (owner) structure editing. Text is edited in place per section. **Each section shows its own change history and track changes directly in the editor** (FR-H4); there is no separate per-node history screen; a document-wide activity feed is available as a secondary view.
