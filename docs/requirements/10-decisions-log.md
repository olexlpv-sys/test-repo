...
| Q19 | Load on the current (weak) environment | **2 concurrent actors, ≤ 10 s response**; tests run with at most 2 parallel workers; single-request perf tests allow 1.5 × the target (`DOCHUB_PERF_FACTOR`). Production targets (Q14) unchanged — [12-load-and-performance](12-load-and-performance.md) NFR-L4a |
| Q20 | Track-changes attribution of text fragments moved between paragraphs | **Accepted limitation** (product owner, Q14 review round 9): moves are tracked per paragraph; when a fragment moved into another paragraph is later split or merged by someone else, the credited author may be the restructurer. The shown text changes are always correct. Revisit with explicit fragment-move tracking if users report it. |
