...
- [ ] Direct SQL `UPDATE` of content in a signed version → `modifiedAfterSigning = true` for that version; a Signed version with a reconciliation finding but `TamperedAt` cleared still reports `true`; a Draft never reports it.
