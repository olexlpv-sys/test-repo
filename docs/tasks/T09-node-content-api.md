...
6a. The content `PUT` updates `ContentJson` **and** all derived columns in one statement, setting `DerivedStale = 0` (so the trigger does not flag it; it runs as `app_api`) and maintains `app.ContentStyleUsage` for the node.
...
8. **Script edits**: support scripts edit `ContentJson` only. The T03 trigger sets `NodeContent.DerivedStale = 1` by the column-based rule in T03 §2a (a `ContentJson` change without all derived columns rewritten in the same statement). A hosted service `DerivedContentRefresher` (every 30 s, batches of 500, filtered index on `DerivedStale = 1`), running as the seeded `system` user (label `OperationContext = 'RebuildDerived'`), re-renders `ContentHtml`, `PlainText`, `ContentHash`, rebuilds `ContentStyleUsage` and clears the flag — it changes derived columns only and runs as the API (`app_api`), so it is not audited and creates no history entry (T03 §2c). This is the **only allowed write to Signed versions** and touches derived columns only — never `ContentJson`, so tamper detection (T07 rule 4) is unaffected. Until refreshed, reads render from `ContentJson` on the fly.
...
- [ ] After a direct SQL update of `ContentJson` (also in a Signed version), `GET …/content?format=html` immediately returns HTML of the new JSON; within 1 min the refresher persists `ContentHtml`/`PlainText`/`ContentHash`, clears `DerivedStale`; the version is still flagged `modifiedAfterSigning`.
