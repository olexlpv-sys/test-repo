...
5. **Audit log** *(optional, if time allows; needs a small endpoint `GET /api/admin/audit?table&source&from&to&page` in T11 scope)* — raw `audit.ChangeLog` viewer with filters, highlighting `Source = Script`.
