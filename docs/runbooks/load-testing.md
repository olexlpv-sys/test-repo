...
- Measured on the development environment (4 vCPU, SQL Server in a container): ~2 300 rows/s (scale 0.003 in 1 min, i.e.
  ...
  on the NFR-L11 Azure SQL tier: `--max-minutes 5` (0.1) or `--max-minutes 30` (1.0) makes the run fail when it takes longer.
- A failing load (log full, throttling) ends the run with its error; nothing is left half-waiting.
...
|---|---|---|
| `small` (default) | 2 concurrent users, 60 s | p99 ≤ 10 s per endpoint, 0 failures (NFR-L4a); mix and NFR-L5 budgets reported |
| `production` | 20 req/s, 30 min | p99 ≤ 3 s per endpoint, 0 failures (NFR-L4); mix 70/20/10 ± 5; every NFR-L5 budget |
| `burst` | 40 req/s, 1 min | 0 failures, 0 × 5xx; mix 70/20/10 ± 5 |
...
node history → compare with the latest signed version), other (comments, sign + withdraw, admin audit reads, new draft + discard,
PDF export ≈ 1 %). Every request is recorded: a request still running at the end of the run is awaited, one over 30 s is a failed
request (timeout), and an error in a flow itself is a failure too. Half way through, the ledger reconciliation runs: it must
finish in < 10 min with 0 findings — and `audit.ReconciliationFinding` must be empty afterwards (findings of a scheduled run of the
deployed API count too) — and the requests around it must stay within the limit.
...
```
20 probes of "main window ready" and "document form first section visible" over an emulated office connection (40 ms latency,
20/5 Mbit/s; `DOCHUB_PROBE_LATENCY_MS`, `DOCHUB_PROBE_DOWN_MBPS`, `DOCHUB_PROBE_UP_MBPS`); the p95 of each must be ≤ 3 s.
...
runtime metrics (GC, memory) go to the OTLP collector.
